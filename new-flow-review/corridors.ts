...
export interface ServiceOption {
  ...
  name:       string
  days:       string
  priceUSD?:  number   // international
  priceNGN?:  number   // domestic NG base (per kg above 1kg)
  priceGHS?:  number   // domestic GH base
  priceKES?:  number   // domestic KE base
  customs?:   number   // USD customs estimate (international only)
  badge?:     string
}
...
export interface CityPair {
  ...
  to:       string
  region:   string
  baseNGN?: number    // flat rate base price NGN
  baseGHS?: number
  baseKES?: number
  days:     string
}
...
export const INTERNATIONAL_CORRIDORS: Record<InternationalCorridorId, {
  ...
  'CN-NG': {
    ...
    transitWindow: '5–9 business days (air) / 25–35 days (sea)',
...
// Pricing: base flat rate + 200 NGN per kg over 1kg
